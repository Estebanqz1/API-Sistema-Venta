...
namespace SistemaVenta.API.Controllers
{
    ...
    public class ProductoController : ControllerBase
    {
        ...
        public async Task<IActionResult> Lista()
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _productoServicio.Lista();
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Guardar([FromBody] ProductoDTO producto)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _productoServicio.Crear(producto);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Editar([FromBody] ProductoDTO producto)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _productoServicio.Editar(producto);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Elimina(int id)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _productoServicio.Eliminar(id);
            }
            catch (Exception ex)
            {
                rsp.status = false;
