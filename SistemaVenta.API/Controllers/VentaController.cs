...
namespace SistemaVenta.API.Controllers
{
    ...
    public class VentaController : ControllerBase
    {
        ...
        public async Task<IActionResult> Guardar([FromBody] VentaDTO venta)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _ventaServicio.Registrar(venta);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Historial(string buscarPor, String? numeroVenta, string? fechaInicio, string? fechaFin)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _ventaServicio.Historial(buscarPor,numeroVenta,fechaInicio,fechaFin);
            }
            catch (Exception ex)
            {
                rsp.status = false;
        ...
        public async Task<IActionResult> Reporte(string? fechaInicio, string? fechaFin)
        {
            ...
            {
                rsp.status = true;
                rsp.values = await _ventaServicio.Reporte(fechaInicio, fechaFin);
            }
            catch (Exception ex)
            {
                rsp.status = false;
